# Developed a machine learning model that can predict whether people have diabetes, when their characteristics are specified with CART algorithm.
...
# Two parameters that will prevent overfitting:
# min_samples_split: 2
# max_depth: None
...
# Extracting in text format of Decision Rules
...
# Extracting SQL codes of Decision Rules

print(skompile(cart_final.predict).to('sqlalchemy/sqlite'))


# Extracting in excel format of Decision Rules
...
# Prediction for two different new observation units:
...
x = [12, 13, 20, 23, 4, 55, 12, 7] # run the model for the new observation unit below and make a prediction
cart_model_from_disc.predict(pd.DataFrame(x).T) # we have to convert it to dataframe!
# output: array([1], dtype=int64) # prediction of the probability that random user will have diabetes
